...
void badapt_jrn_layer_s_setup( badapt_jrn_layer_s* o, sz_t size_input, sz_t size_hidden, sz_t size_output )
{
    bmath_vf3_s_set_size( &o->v_x, size_input );
    bmath_vf3_s_set_size( &o->v_c, size_output );
    bmath_vf3_s_set_size( &o->v_h, size_hidden );
    bmath_vf3_s_set_size( &o->v_o, size_output );
...
void badapt_jrn_arr_layer_s_setup( badapt_jrn_arr_layer_s* o, sz_t size_arr, sz_t size_input, sz_t size_hidden, sz_t size_output )
{
    badapt_jrn_arr_layer_s_set_size( o, size_arr );
    for( sz_t i = 0; i < o->arr_size; i++ )
    {
        if( !o->arr_data[ i ] ) o->arr_data[ i ] = badapt_jrn_layer_s_create();
        badapt_jrn_layer_s_setup( o->arr_data[ i ], size_input, size_hidden, size_output );
    }
}

// ---------------------------------------------------------------------------------------------------------------------

void badapt_jrn_arr_layer_s_rotate( badapt_jrn_arr_layer_s* o )
{
    if( o->arr_size <= 1 ) return;

    badapt_jrn_layer_s* last_layer = o->arr_data[ o->arr_size - 1 ];
...
void badapt_jrn_s_minfer( badapt_jrn_s* o, const bmath_vf3_s* in, bmath_vf3_s* out )
{
    ...
    {
        badapt_jrn_arr_layer_s_setup( &o->arr_layer, o->size_unfolded, o->size_input, o->size_hidden, o->size_output );
    }

    badapt_jrn_arr_layer_s_rotate( &o->arr_layer );
    badapt_jrn_layer_s* prev_layer = o->arr_layer.arr_data[ 1 % o->arr_layer.arr_size ];
    badapt_jrn_layer_s* this_layer = o->arr_layer.arr_data[ 0 ];

    bmath_vf3_s_cpy(               in, &this_layer->v_x );
    bmath_vf3_s_cpy( &prev_layer->v_o, &this_layer->v_c );
    ...
    bmath_mf3_s_mul_vec     ( &o->w_oh, &this_layer->v_h,  &this_layer->v_o );
    badapt_activator_a_infer(  o->a_o, &this_layer->v_o,  &this_layer->v_o );

    if( out ) bmath_vf3_s_cpy( &this_layer->v_o, out );
}
...
{
    badapt_jrn_layer_s* layer0 = o->arr_layer.arr_data[ 0 ];
    bmath_vf3_s_set_size( &o->v_go, layer0->v_o.size );
    ...
    {
        badapt_jrn_layer_s* layer = o->arr_layer.arr_data[ i ];
        badapt_activator_a_adapt_defer( o->a_o, &o->v_go, &o->v_go, &layer->v_o );

        bmath_mf3_s_htp_mul_vec( &o->w_oh, &o->v_go, &o->v_gh );      // W^T * GO -> GH
        bmath_mf3_s_add_opd( &o->gw_oh, &o->v_go, &layer->v_h, &o->gw_oh );

        if( ( i == 0 ) && grad_in ) bmath_mf3_s_htp_mul_vec( &o->w_hx, &o->v_gh, grad_in ); // W^T * GH -> grad_in
        ...
        bmath_vf3_s_cpy( &o->v_gc, &o->v_go ); // GC -> GO
    }

    badapt_activator_a_adapt_apply( o->a_o, o->dynamics.epsilon );
    badapt_activator_a_adapt_apply( o->a_h, o->dynamics.epsilon );
    bmath_mf3_s_mul_scl_f3_add( &o->gw_oh, o->dynamics.epsilon, &o->w_oh, &o->w_oh );
    ...
    {
        bmath_mf3_s_mul_scl_f3( &o->w_oh, ( 1.0 - o->dynamics.epsilon * o->dynamics.lambda_l2 ), &o->w_oh );
...
badapt_adaptive* badapt_jrn_builder_s_build( const badapt_jrn_builder_s* o )
{
    ...
    bmath_mf3_s_set_size( &ern->w_hc, o->size_hidden, o->size_output );
    ...
    bmath_mf3_s_set_size( &ern->gw_oh, ern->w_oh.rows, ern->w_oh.cols );
    bmath_mf3_s_zro( &ern->gw_hx );
    bmath_mf3_s_zro( &ern->gw_hc );
    bmath_mf3_s_zro( &ern->gw_oh );
