...
static bcore_self_s* arg_signature_s_create_self( void )
{
    ...
    bcore_self_s* self = BCORE_SELF_S_BUILD_PARSE_SC( def, bclos_signature_arg_s );
    return self;
}
...
bclos_signature_s* bclos_signature_s_parse_from_source( sr_s source )
{
    ...
    bcore_source_r_parse_fa( &source, " #name", name );
    ...
    {
        if( s->size > 0 ) bcore_source_r_parse_fa( &source, " ," );
        bclos_signature_arg_s arg;
        arg.is_const = bcore_source_r_parse_bl_fa( &source, " #?w'const'" );
        bcore_source_r_parse_fa( &source, " #name ", name );
...
static bcore_self_s* sig_s_create_self( void )
{
    ...
    bcore_self_s* self = BCORE_SELF_S_BUILD_PARSE_SC( def, bclos_signature_s );
    return self;
}
