...
sr_s bclos_procedure_s_call( bclos_procedure_s* o, bclos_env_s* env, const bclos_args_s* args )
{
    ...
    if( o->sig )
    {
        ...
        for( sz_t i = 0; i < o->sig->size; i++ )
        {
            ...
            bclos_env_s_set( local, sig_arg.name, arg_obj );
        }
    }
...
{
    sc_t def = "bclos_procedure_s = { aware_t _; bclos_statement_s * [] arr; bclos_signature_s* sig; private bclos_env_s * lexcal; }";
...
static sr_s test_add( vc_t o, bclos_env_s* env, const bclos_args_s* args )
{
    assert( args->size >= 2 );
...
static sr_s test_mul( vc_t o, bclos_env_s* env, const bclos_args_s* args )
{
    assert( args->size >= 2 );
    assert( sr_s_type( &args->data[ 0 ] ) == sr_s_type( &args->data[ 1 ] ) );
    vc_t a0 = args->data[ 0 ].o;
