/// Author & Copyright (C) 2017 Johannes Bernhard Steffens. All rights reserved.

#include "bcore_quicktypes.h"
#include "bclos_quicktypes.h"
#include "bclos_environment.h"

void bclos_environment_s_init( bclos_environment_s* o )
{
    bcore_memzero( o, sizeof( *o ) );
}

void bclos_environment_s_down( bclos_environment_s* o )
{
    bcore_inst_typed_down( TYPEOF_bclos_environment_s, o );
}

void bclos_environment_s_copy( bclos_environment_s* o, const bclos_environment_s* src )
{
    bcore_inst_typed_copy( TYPEOF_bclos_environment_s, o, src );
}

bclos_environment_s* bclos_environment_s_create()
{
    return bcore_inst_typed_create( TYPEOF_bclos_environment_s );
}

bclos_environment_s* bclos_environment_s_clone( const bclos_environment_s* o )
{
    return bcore_inst_typed_clone( TYPEOF_bclos_environment_s, o );
}

void bclos_environment_s_discard( bclos_environment_s* o )
{
    bcore_inst_typed_discard( TYPEOF_bclos_environment_s, o );
}

sr_s* bclos_environment_s_get( bclos_environment_s* o, tp_t name )
{
    sr_s* val = bcore_hmap_tp_sr_s_get( &o->map, name );
    if( !val && o->external ) val = bclos_environment_s_get( o->external, name );
    return val;
}

sr_s* bclos_environment_s_set( bclos_environment_s* o, tp_t name, sr_s val )
{
    return bcore_hmap_tp_sr_s_set( &o->map, name, val );
}

sr_s bclos_environment_s_remove( bclos_environment_s* o, tp_t name )
{
    return bcore_hmap_tp_sr_s_remove( &o->map, name );
}

bcore_flect_self_s* env_s_create_self( void )
{
    sc_t def =
    "bclos_environment_s = "
    "{ "
        "aware_t _;"
        "private bclos_environment_s* external;"
        "bcore_hmap_tp_sr_s map;"
    "}";

    bcore_flect_self_s* self = bcore_flect_self_s_build_parse_sc( def, sizeof( bclos_environment_s ) );
    return self;
}

vd_t bclos_environment_signal( tp_t target, tp_t signal, vd_t object )
{
    if( target != typeof( "all" ) && target != typeof( "bclos_env" ) ) return NULL;

    if( signal == typeof( "init1" ) )
    {
        bcore_flect_define_creator( typeof( "bclos_environment_s" ), env_s_create_self );
    }

    return NULL;
}

