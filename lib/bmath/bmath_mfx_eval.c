...
st_s* bmath_mfx_eval_result_s_to_string( const bmath_mfx_eval_result_s* o, st_s* string )
{
    ...
    if( o->dev_a > 0 ) st_s_push_fa( string, "dev_a ..... #<f3_t>\n", o->dev_a );
    if( o->dev_u > 0 ) st_s_push_fa( string, "dev_u ..... #<f3_t>\n", o->dev_u );
    if( o->dev_v > 0 ) st_s_push_fa( string, "dev_v ..... #<f3_t>\n", o->dev_v );
    if( o->dev_m > 0 ) st_s_push_fa( string, "dev_m ..... #<f3_t>\n", o->dev_m );
...
static f3_t dev_otn( const bmath_mfx_eval_s* o, const bmath_mf3_s* u )
{
    BLM_INIT();
    if( o->prefer_eps_eval )
    {
        BLM_RETURNV( f3_t, bmath_mf3_s_dev_otn( u ) );
    }
    else
    {
        bmath_mf3_s* m = BLM_CREATE( bmath_mf3_s );
        if( u->rows <= u->cols )
        {
            bmath_mf3_s_set_size( m, u->rows, u->rows );
            bmath_mf3_s_omp_mul_htp( u, u, m );
        }
        else
        {
            bmath_mf3_s_set_size( m, u->cols, u->cols );
            bmath_mf3_s_omp_htp_mul( u, u, m );
        }
        BLM_RETURNV( f3_t, bmath_mf3_s_dev_one( m ) );
    }
    BLM_DOWN();
}
    ...
    if( fp_type == TYPEOF_bmath_fp_mf2_s_mul )
    {
        ...
        if( o->test1 )
        {
            ...
            if( o->assert_all )
            {
                ...
                if( o->prefer_eps_eval ) bmath_mf3_s_mul_esp( m0, m1, m3 ); else bmath_mf3_s_omp_mul( m0, m1, m3 );
                ...
                r->dev_m   = bmath_mf3_s_dev_equ( m2, m3 );
            }
        }
    }
...
static void run_mul_htp( const bmath_mfx_eval_s* o, tp_t fp_type, fp_t fp, bmath_mfx_eval_result_s* res )
{
    ...
    if( fp_type == TYPEOF_bmath_fp_mf2_s_mul_htp )
    {
        ...
        if( o->test0 )
        {
            ...
            if( o->assert_all )
            {
                if( o->prefer_eps_eval ) bmath_mf3_s_mul_htp_esp( m0, m0, m3 ); else bmath_mf3_s_omp_mul_htp( m0, m0, m3 );
                if( o->create_a_log ) bmath_mf3_s_to_string( m2, &r->a_log );
                if( o->create_a_log ) bmath_mf3_s_to_string( m3, &r->a_log );
                eval_s_create_image_file( o, m3, &o->a_img_file );
        ...
        if( o->test1 )
        {
            ...
            if( o->assert_all )
            {
                if( o->prefer_eps_eval ) bmath_mf3_s_mul_htp_esp( m0, m1, m3 ); else bmath_mf3_s_omp_mul_htp( m0, m1, m3 );
                if( o->create_a_log ) bmath_mf3_s_to_string( m2, &r->a_log );
                if( o->create_a_log ) bmath_mf3_s_to_string( m3, &r->a_log );
                eval_s_create_image_file( o, m3, &o->a_img_file );
                r->assert_m = r->assert_m && bmath_mf3_s_is_near_equ( m2, m3, o->near_limit_f2 );
    ...
    if( fp_type == TYPEOF_bmath_fp_mf2_s_htp_mul )
    {
        ...
        if( o->test0 )
        {
            ...
            if( o->assert_all )
            {
                ...
                {
                    bmath_mf3_s_omp_htp_mul( m0, m0, m3 );
                }
        ...
        if( o->test1 )
        {
            ...
            if( o->assert_all )
            {
                ...
                {
                    bmath_mf3_s_omp_htp_mul( m0, m1, m3 );
                }
    ...
    if( fp_type == TYPEOF_bmath_fp_mf2_s_htp_mul_htp )
    {
        ...
        if( o->test1 )
        {
            ...
            if( o->assert_all )
            {
                ...
                {
                    bmath_mf3_s_omp_htp_mul_htp( m0, m1, m3 );
                }
...
static void run_uav( const bmath_mfx_eval_s* o, tp_t fp_type, fp_t fp, bmath_mfx_eval_result_s* res )
{
    ...
    if( o->assert_all && o->test1 )
    {
        r->dev_u = dev_otn( o, u );
        r->dev_v = dev_otn( o, v );
        r->assert_u = r->dev_u < near_limit;
        r->assert_v = r->dev_v < near_limit;
        ...
        else
        {
            bmath_mf3_s* m3 = BLM_CREATE( bmath_mf3_s );
            bmath_mf3_s_set_size( m3, u->rows, a->cols );
            bmath_mf3_s_omp_mul( u, a, m3 );
            bmath_mf3_s_omp_mul_htp( m3, v, m2 );
        }

        r->assert_m = bmath_mf3_s_is_near_equ( m0, m2, near_limit );
        r->dev_m   = bmath_mf3_s_dev_equ( m0, m2 );
    }
...
static void run_av( const bmath_mfx_eval_s* o, tp_t fp_type, fp_t fp, bmath_mfx_eval_result_s* res )
{
    ...
    if( o->assert_all && o->test1 )
    {
        ...
        {
            bmath_mf3_s_omp_mul_htp( a, v, m2 );
        }
...
static void run_inv( const bmath_mfx_eval_s* o, tp_t fp_type, fp_t fp, bmath_mfx_eval_result_s* res )
{
    ...
    r->dev_m   = bmath_mf3_s_dev_one( m2 );
    if( res ) bmath_mfx_eval_result_s_copy( res, r );
...
static void run_piv( const bmath_mfx_eval_s* o, tp_t fp_type, fp_t fp, bmath_mfx_eval_result_s* res )
{
    ...
    // rank deficit --> verify Moore-Penrose-Conditions
    {
        ...
        r->dev_u   = bmath_mf3_s_dev_equ( m0, m2 );
        ...
        r->dev_v   = bmath_mf3_s_dev_equ( a, m2 );
    }
