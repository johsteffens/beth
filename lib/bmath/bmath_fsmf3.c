...
{
    bmath_fsmf3_s_set_size_splicing( o, rows, cols );
    bmath_fsmf3_s_set_size_data( o, rows * cols );
...
void bmath_fsmf3_s_fit_size_data( bmath_fsmf3_s* o )
{
    ...
    bmath_fsmf3_s_set_size_data( o, max_idx + 1 );
}
...
void bmath_fsmf3_s_set_random( bmath_fsmf3_s* o, bl_t hsm, bl_t pdf, uz_t rd, f3_t density, f3_t min, f3_t max, u2_t* p_rval )
{
    ...
    bmath_fsmf3_s_cpy_dfl_from_mf3( o, m1 );
    BCORE_LIFE_DOWN();
}
...
void bmath_fsmf3_s_set_splicing_for_convolution_2d( bmath_fsmf3_s* o, sz_t rows_in, sz_t cols_in, sz_t rows_kernel, sz_t cols_kernel, sz_t row_step, sz_t col_step )
{
    ...
    bmath_fsmf3_s_set_size_splicing( o, o->rows, o->cols );
...
bl_t bmath_fsmf3_s_is_near_equ( const bmath_fsmf3_s* o, const bmath_fsmf3_s* op, f3_t max_dev )
{
...
bl_t bmath_fsmf3_s_is_near_zro( const bmath_fsmf3_s* o, f3_t max_dev )
{
    for( sz_t row = 0; row <= o->rows; row++ )
    {
        for( sz_t col = 0; col <= o->cols; col++ )
...
static vd_t selftest( void )
{
    ...
    BCORE_LIFE_CREATE( bmath_fsmf3_s, sm1 );

    {
        ...
        bmath_fsmf3_s_set_size( sm1, rows, cols );
        bmath_mf3_s_set_random( m1, false, false, 0, 1.0, -1.0, 1.0, &rval );
        bmath_fsmf3_s_cpy_dfl_from_mf3( sm1, m1 );
        bmath_fsmf3_s_cpy_ifl_to_mf3(   sm1, m2 );
...
{
    switch( bcore_signal_s_handle_type( o, typeof( "bmath_fsmf3" ) ) )
    {
        case TYPEOF_init1:
        {
        }
