...
f3_t bmath_mf3_s_is_fdev_equ( const bmath_mf3_s* o, const bmath_mf3_s* op )
{
    ...
    for( sz_t i = 0; i < o->rows; i++ )
    {
        ...
        for( sz_t j = 0; j < o->cols; j++ ) sum += f3_sqr( ai[ j ] - bi[ j ] );
    }
...
f3_t bmath_mf3_s_is_fdev_zro( const bmath_mf3_s* o )
{
    ...
    for( sz_t i = 0; i < o->rows; i++ )
    {
        ...
        for( sz_t j = 0; j < o->cols; j++ ) sum += f3_sqr( oi[ j ] );
    }
    return ( sum > 0 ) ? f3_srt( sum ) : 0;
}
...
void bmath_mf3_s_decompose_lbd( bmath_mf3_s* u, bmath_mf3_s* a, bmath_mf3_s* v )
{
    ...
    {
        bmath_mf3_s_one( u );
        for( sz_t j = a->rows - 2; j < a->rows; j-- )
        {
            for( sz_t k = j + 1; k < a->rows - 1; k++ )
            {
                f3_t rho = 0;
                f3_t_swap( &a->data[ j + a->stride * ( k + 1 ) ], &rho );
                bmath_grt_f3_s_init_from_rho( &gr, -rho );
