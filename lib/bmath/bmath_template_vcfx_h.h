...
static inline void BCATU(bmath_vcfx_s,set_r)( bmath_vcfx_s* o, uz_t index, fx_t r ) { assert( index < o->size ); o->data[ index ].r = r; }
static inline void BCATU(bmath_vcfx_s,set_i)( bmath_vcfx_s* o, uz_t index, fx_t i ) { assert( index < o->size ); o->data[ index ].i = i; }

/// sets real values from vector
static inline void BCATU(bmath_vcfx_s,set_vr)( bmath_vcfx_s* o, const bmath_vfx_s* vec ) { uz_t sz = uz_min( o->size, vec->size ); for( uz_t i = 0; i < sz; i++ ) o->data[ i ].r = vec->data[ i ]; }

/// sets imag values from vector
static inline void BCATU(bmath_vcfx_s,set_vi)( bmath_vcfx_s* o, const bmath_vfx_s* vec ) { uz_t sz = uz_min( o->size, vec->size ); for( uz_t i = 0; i < sz; i++ ) o->data[ i ].i = vec->data[ i ]; }
...
er_t BCATU(bmath_vcfx_s,copy_typed)( bmath_vcfx_s* o, tp_t type, vc_t src );
