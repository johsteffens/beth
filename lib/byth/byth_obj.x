...
func (:s) py_dict_get_obj
{
    ...
    {
        byth_py_check_error();
        EM_ERR_fa( "Error retrieving Item from PyDict." );
        = NULL;
    }
    = obj.fork();
}
...
func (:s) py_list_get_obj
{
    ...
    {
        byth_py_check_error();
        EM_ERR_fa( "Error retrieving Item from PyList." );
        = NULL;
    }
    = obj.fork();
}
...
func (:s) setup_from_t_inst
{
    ...
    if( py_object != Py_None )
    {
        ...
        {
            byth_py_check_error();
            = EM_ERR_fa( "Converting '#<sc_t>' failed.", bnameof( type ) );
        }
...

    {
        ...
        {
            byth_py_check_error();
            = EM_ERR_fa( "PyDict_SetItem failed." );
        }
    }
    ...
    if( x_stamp_t_is_pure_array( type ) )
    {
        ...
        for( sz_t i = 0; i < size; i++ )
        {
            ...
            {
                byth_py_check_error();
                err = EM_ERR_fa( "PyList_SetItem failed." );
                break;
            }
        }
        if( err ) = err;
    ...
    else
    {
        ...
        for( sz_t i = 0; i < size; i++ )
        {
            ...
            {
                byth_py_check_error();
                err = EM_ERR_fa( "PyDict_SetItem failed." );
                break;
            }
        }
        if( err ) = err;
...
func (:s) to_t_inst
{
    ...
    switch( type )
    {
        ...
        case sc_t~: = EM_ERR_fa( "Cannot convert to sc_t." );
        case st_s~:
        {
            if( !o.is_py_unicode() ) = EM_ERR_fa( "Attempt to convert to st_s from non unicode PyObject" );
            m st_s* st = inst.cast( m st_s* );
            m PyObject* py_unicode = PyUnicode_AsEncodedString( o.py_object, "utf-8", "strict" ); // create UTF8 encoding
            if( !py_unicode )
            {
                byth_py_check_error();
                = EM_ERR_fa( "PyUnicode_AsEncodedString error" );
            }
    ...
    if( o.is_py_dict() )
    {
        if( x_stamp_t_is_pure_array( type ) && o.py_dict_contains( "_list_" ) )
        {
            ...
            {
                byth_py_check_error();
                = EM_ERR_fa( "Error retrieving PyList from PyDict." );
            }
...
func (:s) setup_from_hmap
{
    ...
    {
        byth_py_check_error();
        = EM_ERR_fa( "Creating PyDict failed." );
    }
    ...
    for( sz_t i = 0; i < size; i++ )
    {
        ...
        if( key )
        {
            ...
            if( !sc_key ) = EM_ERR_fa( "hmap contains a key not registered in the global name map." );
...
{
    hmap.clear();
    if( !o.is_py_dict() ) EM_ERR_fa( "py_object is no dictionary" );
    ...
    {
        if( !PyUnicode_Check( py_key ) ) = EM_ERR_fa( "py_key is not PyUnicode" );
