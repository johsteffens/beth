...
// o_self can be NULL
static bcore_interpreter_s* create_from_self( const bcore_flect_self_s* t_self, const bcore_flect_self_s* o_self )
{
    bcore_interpreter_s* o = interpreter_s_create();
    o->t_type = t_self->type;
    o->o_type = o_self ? o_self->type : 0;
    o->interpret_body = o_self ? ( bcore_fp_interpret_body )bcore_flect_self_s_try_external_fp( o_self, typeof( "bcore_fp_interpret_body" ) ) : NULL;
    if( !o->interpret_body ) o->interpret_body = ( bcore_fp_interpret_body )bcore_flect_self_s_get_external_fp( t_self, typeof( "bcore_fp_interpret_body" ) );
    o->interpret_object = o_self ? ( bcore_fp_interpret_object )bcore_flect_self_s_try_external_fp( o_self, typeof( "bcore_fp_interpret_object" ) ) : NULL;
    if( !o->interpret_object ) o->interpret_object = ( bcore_fp_interpret_object )bcore_flect_self_s_get_external_fp( t_self, typeof( "bcore_fp_interpret_object" ) );
    return o;
}
