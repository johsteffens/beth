...
static void bcore_plant_stamp_s_parse( bcore_plant_stamp_s* o, bcore_plant_group_s* group, bcore_source* source )
{
    ...
    while( !bcore_source_a_eos( source ) && !bcore_source_a_parse_bl_fa( source, " #?'}'" ) )
    {
        if( bcore_source_a_parse_bl_fa( source, " #?w'func'" ) )
        {
            ...
            bcore_source_point_s_set( &func->source_point, source );
    ...
    for( sz_t i = 0; i < o->funcs.size; i++ )
    {
        ...
        if( bcore_plant_compiler_s_item_exists( plant_compiler_g, func->type ) )
        {
            ...
            else if( *(aware_t*)item == TYPEOF_bcore_plant_signature_s )
            {
                ...
                bcore_sink_a_push_fa( sink, "#<sc_t>", signature->mutable ? "" : "const " );
                bcore_sink_a_push_fa( sink, "#<sc_t>* o", o->name.sc );
                bcore_plant_args_s_expand( &signature->args, sink );
                bcore_sink_a_push_fa( sink, " )" );
...
//    st_s* source_path_c      = BCORE_LIFE_A_PUSH( st_s_create_fa( "#<sc_t>.c", source_path_n->sc ) );
    ...
    // parsing *.c files is generally not helpful  (hardly any plant code can reside in *.c files)
...
{
    bcore_sink_a_push_fa( sink, "/** This file was generated by the plant-compiler.\n" );
    bcore_sink_a_push_fa( sink, " *\n" );
    bcore_sink_a_push_fa( sink, " *  Copyright and License:\n" );
    bcore_sink_a_push_fa( sink, " *    Code generated by the beth-plant compiler inherits the copyright and license\n" );
    bcore_sink_a_push_fa( sink, " *    of the beth-plant source code, which is contained in files listed here:\n" );
    bcore_sink_a_push_fa( sink, " *\n" );
    for( sz_t i = 0; i < o->size; i++ )
    {
