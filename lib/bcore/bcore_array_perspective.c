...
static vd_t set_item_c_typed_link( const bcore_array_s* p, vd_t o, sz_t index, vc_t src )
{
    ...
    if( index >= arr->size ) set_size( p, o, index + 1 );
    vd_t* dst = &arr->data[ index ];
    bcore_object_typed_discard( arr->type, *dst );
...
static vd_t set_item_c_aware_link( const bcore_array_s* p, vd_t o, sz_t index, vc_t src )
{
    ...
    if( index >= arr->size ) set_size( p, o, index + 1 );
    vd_t* dst = &arr->data[ index ];
    bcore_object_aware_discard( *dst );
...
static vd_t set_item_d_static( const bcore_array_s* p, vd_t o, sz_t index, vd_t src )
{
    ...
    const bcore_object_s* object_p = p->item_p;
    vd_t dst = ( u0_t* )arr->data + object_p->size * index;
    if( !object_p->down_flat ) object_p->down( object_p, dst );
    object_p->copy( object_p, dst, src );
...
static bool linked_data( const bcore_array_s* p )
{
    switch( p->caps_type )
    {
...
static sz_t get_unit_size( const bcore_array_s* p )
{
    switch( p->caps_type )
    {
...
static vc_t max( const bcore_array_s* p, vc_t o, bcore_fp_cmp cmp, s2_t order )
{
    sz_t size = p->get_size( p, o );
    if( p->linked_data( p ) )
    {
    ...
    {
        vc_t src = p->get_c_data( p, o );
        vc_t cur = src;
        sz_t step = p->get_unit_size( p );
        for( sz_t i = 1; i < size; i++ )
        {
            src = ( u0_t* )src + step;
...
static sz_t max_index( const bcore_array_s* p, vc_t o, bcore_fp_cmp cmp, s2_t order )
{
    sz_t size = p->get_size( p, o );
    if( p->linked_data( p ) )
    {
...
static void sort( const bcore_array_s* p, vd_t o, bcore_fp_cmp cmp, s2_t order )
{
    sz_t size = p->get_size( p, o );
    if( p->linked_data( p ) )
    {
        vc_t* data = ( vc_t* )p->get_d_data( p, o );
        vc_t* buf = bcore_un_alloc( sizeof( vd_t ), NULL, 0, size >> 1, NULL );
        buf_sort_link( data, size, buf, cmp, order );
        bcore_un_alloc( sizeof( vd_t ), buf, size >> 1, 0, NULL );
    }
    else
    {
        vd_t data = p->get_d_data( p, o );
        const bcore_object_s* item_p = p->item_p;
        sz_t unit_size = item_p->size;

        sz_t size1 = size >> 1;
        vd_t buf = bcore_un_alloc( unit_size, NULL, 0, size1, NULL );
        for( sz_t i = 0; i < size1; i++ ) item_p->init( item_p, ( u0_t* )buf + i * unit_size );

        buf_sort_empl( item_p, data, size, buf, cmp, order );

        for( sz_t i = 0; i < size1; i++ ) item_p->down( item_p, ( u0_t* )buf + i * unit_size );
        bcore_un_alloc( unit_size, buf, size1, 0, NULL );
    }
}
...
{
    bcore_flect_define_self_d( bcore_flect_self_s_build_parse_sc( "string_array_s = { aware_t _; bcore_string_s [] string_arr; }" ) );
    const bcore_array_s* arr_p = bcore_array_s_get_typed( typeof( "string_array_s" ) );

    vd_t arr = bcore_object_typed_create( typeof( "string_array_s" ) );
    ...
    arr_p->sort( arr_p, arr, ( bcore_fp_cmp )bcore_string_s_cmp_string, 1 );

    bcore_string_s_pushf( log, "size = %zu\n", arr_p->get_size( arr_p, arr ) );
    bcore_string_s_pushf( log, "space = %zu\n", arr_p->get_space( arr_p, arr ) );
    for( sz_t i = 0; i < arr_p->get_size( arr_p, arr ); i++ )
    {
        ...
        if( string )
        {
            bcore_string_s_pushf( log, "index %zu: %s\n", i, string->sc );
        }
    }

    bcore_string_s_pushf( log, "max index %zu\n", arr_p->max_index( arr_p, arr, ( bcore_fp_cmp )bcore_string_s_cmp_string, 1 ) );
