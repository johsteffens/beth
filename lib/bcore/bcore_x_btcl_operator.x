...
/** BTCL: Exportable Operators
 *  The operators are cover nearly all BTCL unary, binary, ternary operators, including most builtin functions.
 *  The operators are exportable (normally in the form of a functor) and can be used outside the btcl parser framework.
 *  Highly optmized operators limited to only f3_t numeric types can be found in :operators_f3, :functor_f3
 */
    ...
    switch( o.type )
    {
        ...
        case max~:
        {
            if( bcore_tp_is_numeric( a.type() ) && bcore_tp_is_numeric( b.type() ) )
            {
                if( bcore_tp_is_integer( a.type() ) && bcore_tp_is_integer( b.type() ) )
                {
                    result.const_from_s3( s3_max( a.to_s3(), b.to_s3() ) );
                }
                else
                {
                    result.const_from_f3( f3_max( a.to_f3(), b.to_f3() ) );
                }
                = 0;
            }
        }
        ...
        case min~:
        {
            if( bcore_tp_is_numeric( a.type() ) && bcore_tp_is_numeric( b.type() ) )
            {
                if( bcore_tp_is_integer( a.type() ) && bcore_tp_is_integer( b.type() ) )
                {
                    result.const_from_s3( s3_min( a.to_s3(), b.to_s3() ) );
                }
                else
                {
                    result.const_from_f3( f3_min( a.to_f3(), b.to_f3() ) );
                }
                = 0;
            }
        }
