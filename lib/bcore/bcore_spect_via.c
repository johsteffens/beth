...
sr_s bcore_via_default_iget( const bcore_via_s* p, const bcore_via* o, sz_t index         )
{
...
tp_t bcore_via_default_iget_type( const bcore_via_s* p, const bcore_via* o, sz_t index )
{
    if( index >= p->size ) ERR( "index (%zu) out of range (%zu)", index, p->size );
    const bcore_vitem_s* vitem = &p->vitem_arr[ index ];
    switch( vitem->caps )
...
void bcore_via_default_iset( const bcore_via_s* p, bcore_via* o, sz_t index, sr_s src )
{
    if( index >= p->size ) ERR( "index (%zu) out of range (%zu)", index, p->size );
    const bcore_vitem_s* vitem  = &p->vitem_arr[ index ];
...
void bcore_via_default_iset_s3( const bcore_via_s* p, bcore_via* o, sz_t index, s3_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_s3_t, &val ) ); }
void bcore_via_default_iset_u3( const bcore_via_s* p, bcore_via* o, sz_t index, u3_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_u3_t, &val ) ); }
void bcore_via_default_iset_f3( const bcore_via_s* p, bcore_via* o, sz_t index, f3_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_f3_t, &val ) ); }
void bcore_via_default_iset_sz( const bcore_via_s* p, bcore_via* o, sz_t index, sz_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_sz_t, &val ) ); }
void bcore_via_default_iset_sc( const bcore_via_s* p, bcore_via* o, sz_t index, sc_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_sc_t, &val ) ); }
void bcore_via_default_iset_bl( const bcore_via_s* p, bcore_via* o, sz_t index, bl_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_bl_t, &val ) ); }
void bcore_via_default_iset_tp( const bcore_via_s* p, bcore_via* o, sz_t index, tp_t val ) { bcore_via_default_iset( p, o, index, sr_twc( TYPEOF_tp_t, &val ) ); }

const bcore_array_s* bcore_via_default_iget_array( const bcore_via_s* p, const bcore_via* o, sz_t index )
{
    const bcore_vitem_s* vitem = &p->vitem_arr[ index ];
...
vc_t bcore_via_default_iget_spect( const bcore_via_s* p, const bcore_via* o, sz_t index, tp_t spect_type )
{
...
{
    for( sz_t i = 0; i < p->size; i++ ) if( p->vitem_arr[ i ].name == name ) return true;
    return false;
}
...
sz_t                 NPX(default_nget_index   )( const NPX(s)* p, const bcore_via* o, tp_t n           ) { return vidx(       p, n ); }
...
bl_t bcore_via_default_iis_array( const bcore_via_s* p, const bcore_via* o, sz_t index )
{
    if( index >= p->size ) ERR( "index (%zu) out of range (%zu)", index, p->size );
    return bcore_flect_caps_is_array( p->vitem_arr[ index ].caps );
}

bl_t bcore_via_default_iis_static( const bcore_via_s* p, const bcore_via* o, sz_t index )
{
    if( index >= p->size ) ERR( "index (%zu) out of range (%zu)", index, p->size );
...
bl_t bcore_via_default_iis_link( const bcore_via_s* p, const bcore_via* o, sz_t index )
{
    if( index >= p->size ) ERR( "index (%zu) out of range (%zu)", index, p->size );
    switch( p->vitem_arr[ index ].caps )
    {
...
static st_s* spect_via_selftest( void )
{
    ...
    {
        bcore_via_x_nset_sz( via_specs, typeof( "size"   ),  10 );
        bcore_via_x_nset_u3( via_specs, typeof( "param1" ), 200 );
        bcore_via_x_nset_s3( via_specs, typeof( "" ), -50 );
    }

    ASSERT( *( sz_t* )bcore_via_x_nget( via_specs, typeof( "size"   ) ).o ==  10 );
