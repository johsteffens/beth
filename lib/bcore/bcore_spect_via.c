...
void bcore_via_spect_iset( const bcore_via_s* p, vd_t o, sz_t index, sr_s src )
{
    ...
    switch( vitem->caps )
    {
        ...
        case BCORE_CAPS_STATIC_LINK:
        {
            ...
            if( sr_type( src ) == vitem->type )
            {
                dst->link = sr_is_strong( src ) ? src.o : bcore_inst_spect_clone( inst_p, src.o );
                src = sr_cw( src );
            }
            else
            {
                dst->link = bcore_inst_spect_create_typed( inst_p, sr_type( src ), src.o );
            }
        }
        break;
        ...
        case BCORE_CAPS_TYPED_LINK:
        {
            ...
            if( sr_type( src ) )
            {
                dst->type = sr_type( src );
                dst->link = sr_is_strong( src ) ? src.o : bcore_inst_typed_clone( sr_type( src ), src.o );
                src = sr_cw( src );
            }
        }
        break;
        ...
        case BCORE_CAPS_AWARE_LINK:
        {
            ...
            if( src.o )
            {
                ...
                {
                    dst->link = sr_is_strong( src ) ? src.o : bcore_inst_aware_clone( src.o );
                    src = sr_cw( src );
                }
            }
        }
