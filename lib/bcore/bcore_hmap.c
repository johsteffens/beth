...
static u3_t hash_u3u3_1( u3_t key )
{
...
static u3_t hash_u3u3_2( u3_t key )
{
...
/**********************************************************************************************************************/
// bcore_hnode_u3vd_s, bcore_hmap_u3vd_s

//----------------------------------------------------------------------------------------------------------------------

typedef struct bcore_hnode_u3vd_s
{
    u3_t key;
...
static void bcore_hnode_u3vd_s_init( bcore_hnode_u3vd_s* o )
{
...
static void bcore_hnode_u3vd_s_down( bcore_hnode_u3vd_s* o )
{
...
static void bcore_hnode_u3vd_s_clear( bcore_hnode_u3vd_s* o )
{
    if( o->flag_holds && o->dp ) bcore_inst_a_discard( o->dp );
    o->dp = 0;
    o->flag_holds = 0;
...
static void bcore_hnode_u3vd_s_copy( bcore_hnode_u3vd_s* o, const bcore_hnode_u3vd_s* src )
{
...
static void bcore_hnode_u3vd_s_set_val( bcore_hnode_u3vd_s* o, vd_t val, bool hold )
{
    if( o->flag_holds )
    {
...
static void bcore_hnode_u3vd_s_set_func( bcore_hnode_u3vd_s* o, fp_t func )
{
    if( o->flag_holds )
    {
        if( o->dp ) bcore_inst_a_discard( o->dp );
...
void bcore_hmap_u3vd_s_init( bcore_hmap_u3vd_s* o )
{
    bcore_memzero( o, sizeof( *o ) );
    o->_ = TYPEOF_bcore_hmap_u3vd_s;
    o->h1         = hash_u3u3_1;
    o->h2         = hash_u3u3_2;
    o->h3         = NULL;
}
...
void bcore_hmap_u3vd_s_down( bcore_hmap_u3vd_s* o )
{
    for( uz_t i = 0; i < o->size; i++ ) bcore_hnode_u3vd_s_down( &o->data[ i ] );
    bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), o->data, o->space, 0, NULL );
}

//----------------------------------------------------------------------------------------------------------------------

void bcore_hmap_u3vd_s_copy( bcore_hmap_u3vd_s* o, const bcore_hmap_u3vd_s* src )
{
    for( uz_t i = 0; i < o->size; i++ ) bcore_hnode_u3vd_s_down( &o->data[ i ] );
    if( src->size > o->space )
    {
        o->data = bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), o->data, o->space,  0, &o->space );
        o->data = bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), o->data, 0, src->size, &o->space );
    }
    o->size = src->size;
    for( uz_t i = 0; i < o->size; i++ )
    {
        bcore_hnode_u3vd_s_init( &o->data[ i ] );
        bcore_hnode_u3vd_s_copy( &o->data[ i ], &src->data[ i ] );
    }
...
static void u3vd_set_size( bcore_hmap_u3vd_s* o, uz_t size )
{
    for( uz_t i = 0; i < o->size; i++ ) bcore_hnode_u3vd_s_clear( &o->data[ i ] );

    if( ( size & ( size - 1 ) ) != 0 ) ERR( "size %zu must be a power of two", size );
    if( size > o->space )
    {
        o->data = bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), o->data, o->space, 0, &o->space );
        o->data = bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), o->data, 0, size,  &o->space );
    }
    o->size = size;
    bcore_memzero( o->data, sizeof( bcore_hnode_u3vd_s ) * o->size );
...
BCORE_DEFINE_FUNCTION_CREATE(  bcore_hmap_u3vd_s )
BCORE_DEFINE_FUNCTION_DISCARD( bcore_hmap_u3vd_s )
BCORE_DEFINE_FUNCTION_CLONE(   bcore_hmap_u3vd_s )

//----------------------------------------------------------------------------------------------------------------------

static uz_t u3vd_find( const bcore_hmap_u3vd_s* o, u3_t key ) // returns valid index or o->size
{
...
static bcore_hnode_u3vd_s* u3vd_set( const bcore_hmap_u3vd_s* o, bcore_hnode_u3vd_s node, uz_t depth ) // sets new node
{
...
void bcore_hmap_u3vd_s_set_hash_function( bcore_hmap_u3vd_s* o, uz_t index, bcore_hash_u3u3 hf )
{
...
vd_t* bcore_hmap_u3vd_s_get( const bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return NULL;
    uz_t idx = u3vd_find( o, key );
    return ( idx < o->size ) ? &o->data[ idx ].dp : NULL;
}

//----------------------------------------------------------------------------------------------------------------------

fp_t* bcore_hmap_u3vd_s_getf( const bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return NULL;
    uz_t idx = u3vd_find( o, key );
...
void bcore_hmap_u3vd_s_set( bcore_hmap_u3vd_s* o, u3_t key, vd_t val, bool hold )
{
...

    {
        ...
        if( idx < o->size )
        {
            bcore_hnode_u3vd_s_set_val( &o->data[ idx ], val, hold );
            return;
        }
    }

    {
        bcore_hnode_u3vd_s node;
        bcore_hnode_u3vd_s_init( &node );
        node.key = key;
        bcore_hnode_u3vd_s* p_node = u3vd_set( o, node, 1 );
        if( p_node )
        {
            bcore_hnode_u3vd_s_set_val( p_node, val, hold );
            return;
        }
    }
    ...
    // rehash
    {
        bcore_hnode_u3vd_s* buf_data  = o->data;
        uz_t         buf_size  = o->size;
        uz_t         buf_space = o->space;
        o->depth_limit = ( o->size > 0 ) ? o->depth_limit + 1 : 4;
        o->size        = ( o->size > 0 ) ? o->size * 2        : 8;
        o->data = bcore_u_alloc( sizeof( bcore_hnode_u3vd_s ), NULL, o->size, &o->space );
        bcore_memzero( o->data, sizeof( bcore_hnode_u3vd_s ) * o->size );
        for( uz_t i = 0; i < buf_size; i++ )
        {
            bcore_hnode_u3vd_s* p_node = &buf_data[ i ];
            if( p_node->key ) bcore_hmap_u3vd_s_set( o, p_node->key, p_node->dp, p_node->flag_holds );
        }
        bcore_un_alloc( sizeof( bcore_hnode_u3vd_s ), buf_data, buf_space, 0, NULL );
    }

    bcore_hmap_u3vd_s_set( o, key, val, hold );
}
...
void bcore_hmap_u3vd_s_setf( bcore_hmap_u3vd_s* o, u3_t key, fp_t func )
{
...

    {
        ...
        if( idx < o->size )
        {
            bcore_hnode_u3vd_s_set_func( &o->data[ idx ], func );
            return;
        }
    }

    {
        ...
        if( p_node )
        {
            bcore_hnode_u3vd_s_set_func( p_node, func );
            return;
        }
    }
    ...
    bcore_hmap_u3vd_s_setf( o, key, func );
}

//----------------------------------------------------------------------------------------------------------------------

vd_t bcore_hmap_u3vd_s_remove_h( bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return NULL;
    ...
    if( idx < o->size )
    {
        bcore_hnode_u3vd_s* node = &o->data[ idx ];
        vd_t ret = node->dp;
        node->key = 0;
        node->dp  = 0;
        node->flag_holds = 0;
...
fp_t bcore_hmap_u3vd_s_removef_h( bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return NULL;
...
void bcore_hmap_u3vd_s_remove_d( bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return;
    uz_t idx = u3vd_find( o, key );
    if( idx < o->size )
...
vd_t bcore_hmap_u3vd_s_detach_h( bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return NULL;
...
bool bcore_hmap_u3vd_s_exists( const bcore_hmap_u3vd_s* o, u3_t key )
{
    if( !key ) return false;
    uz_t idx = u3vd_find( o, key );
    return ( idx < o->size );
...
void bcore_hmap_u3vd_s_clear( bcore_hmap_u3vd_s* o )
{
    u3vd_set_size( o, 0 );
}
...
uz_t bcore_hmap_u3vd_s_keys( const bcore_hmap_u3vd_s* o )
{
    uz_t count = 0;
...
uz_t bcore_hmap_u3vd_s_size( const bcore_hmap_u3vd_s* o )
{
    return o->size;
}
...
u3_t bcore_hmap_u3vd_s_idx_key( const bcore_hmap_u3vd_s* o, uz_t idx )
{
    assert( idx < o->size );
...
bool bcore_hmap_u3vd_s_idx_holds( const bcore_hmap_u3vd_s* o, uz_t idx )
{
    assert( idx < o->size );
...
vd_t bcore_hmap_u3vd_s_idx_val( const bcore_hmap_u3vd_s* o, uz_t idx )
{
    assert( idx < o->size );
...
void bcore_hmap_u3vd_s_run_c( const bcore_hmap_u3vd_s* o, vd_t obj, void (*fp)( vd_t obj, u3_t key, vd_t val ) )
{
    for( uz_t i = 0; i < o->size; i++ )
    {
...
void bcore_hmap_u3vd_s_run_d( bcore_hmap_u3vd_s* o, vd_t obj, void (*fp)( vd_t obj, u3_t key, vd_t* val ) )
{
    for( uz_t i = 0; i < o->size; i++ )
    {
        if( o->data[ i ].key > 0 ) fp( obj, o->data[ i ].key, &o->data[ i ].dp );
...
static bcore_self_s* hmap_u3vd_s_create_self( void )
{
    sc_t def = "\
        bcore_hmap_u3vd_s = bcore_inst \
            { \
                aware_t _; \
    ...
    bcore_self_s* self = BCORE_SELF_S_BUILD_PARSE_SC( def, bcore_hmap_u3vd_s );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_init,    "bcore_fp_init",    "init"    );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_down,    "bcore_fp_down",    "down"    );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_copy,    "bcore_fp_copy",    "copy"    );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_create,  "bcore_fp_create",  "create"  );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_clone,   "bcore_fp_clone",   "clone"   );
    bcore_self_s_push_ns_func( self, ( fp_t )bcore_hmap_u3vd_s_discard, "bcore_fp_discard", "discard" );
    return self;
}

//----------------------------------------------------------------------------------------------------------------------

static st_s* hmap_u3vd_s_status( bcore_hmap_u3vd_s* o )
{
    st_s* string = st_s_create();
    uz_t keys = bcore_hmap_u3vd_s_keys( o );
...
void bcore_hmap_u3vd_filltest()
{
    bcore_life_s* life = bcore_life_s_create();
    bcore_hmap_u3vd_s* map = bcore_life_s_push( life, ( bcore_fp_discard )bcore_hmap_u3vd_s_discard, bcore_hmap_u3vd_s_create() );
    ...
    {
        bcore_hmap_u3vd_s_clear( map );
        u3_t key = bcore_lcg00_u3( i + 1 );
        for( uz_t j = 0; j < size; j++ )
        {
            key = bcore_lcg00_u3( key );
            uz_t size1 = map->size;
            bcore_hmap_u3vd_s_set( map, key, 0, false );
            uz_t size2 = map->size;
            if( size2 > size1 )
            {
        ...
        st_s_print_d( hmap_u3vd_s_status( map ) );
    }
...
static st_s* hmap_u3vd_selftest( void )
{
...

    {
        ...
        for( uz_t i = 0; i < cycles; i++ )
        {
            ...
            // set
            bcore_hmap_u3vd_s_set( map, kv.key, kv.val, false );

            // retrieve
            rval1 = bcore_lcg00_u3( rval1 );
            kv = kvbuf[ rval1 % kvbuf_size ];
            vd_t* val_ptr = bcore_hmap_u3vd_s_get( map, kv.key );
            ...
            if( ( ( rval1 >> 10 ) & 1 ) == 1 )
            {
                ...
                kv_s kv = kvbuf[ idx ];
                if( !bcore_hmap_u3vd_s_get( map, kv.key ) )  ERR( "key (%lu) not found", kv.key );
                bcore_hmap_u3vd_s_remove_d( map, kv.key );
                if( bcore_hmap_u3vd_s_get( map, kv.key ) )  ERR( "deleted key still exists (%lu)", kv.key );
                kvbuf_size--;
    ...
    st_s_push_st_d( log, hmap_u3vd_s_status( map ) );
    ...
    for( uz_t j = 0; j < read_cycles; j++ )
    {
        ...
        {
            if( *bcore_hmap_u3vd_s_get( map, kvbuf[ i ].key ) != kvbuf[ i ].val ) ERR( "value mismatch (%lu vs %lu)", kvbuf[ i ].key, kvbuf[ i ].val );
        }
    }
    time = clock() - time;
...
static bcore_self_s* hnode_tpuz_s_create_self( void )
{
    sc_t def =
    "bcore_hnode_tpuz_s = bcore_inst"
    "{"
        "tp_t key;"
        "uz_t val;"
    "}";
...
void bcore_hmap_tpuz_s_copy( bcore_hmap_tpuz_s* o, const bcore_hmap_tpuz_s* src )
{
    ...
    o->nodes = bcore_u_memcpy( sizeof( bcore_hnode_tpuz_s ), o->nodes, src->nodes, src->size );
    o->size  = src->size;
...
static uz_t tpuz_find( const bcore_hmap_tpuz_s* o, tp_t key ) // returns valid index or o->size
{
    if( o->size == 0 ) return o->size;
    uz_t mask = o->size - 1;
...
uz_t tpuz_set( bcore_hmap_tpuz_s* o, bcore_hnode_tpuz_s node, uz_t depth ) // sets new node, returns valid index on success; o->size otherwise
{
    uz_t size = o->size;
    ...
    {
        o->flags[ idx1 ] = true;
        if( tpuz_set( o, o->nodes[ idx1 ], depth + 1 ) < size )
        {
            o->flags[ idx1 ] = false;
            o->nodes[ idx1 ] = node;
            return idx1;
    ...
    {
        o->flags[ idx2 ] = true;
        if( tpuz_set( o, o->nodes[ idx2 ], depth + 1 ) < size )
        {
            o->flags[ idx2 ] = false;
            o->nodes[ idx2 ] = node;
            return idx2;
...
uz_t* bcore_hmap_tpuz_s_set( bcore_hmap_tpuz_s* o, tp_t key, uz_t val )
{
    ...
    idx = tpuz_set( o, ( bcore_hnode_tpuz_s ){ .key = key, .val = val }, 1 );
    if( idx < o->size ) return &o->nodes[ idx ].val;

    // rehash
    {
        o->flags = bcore_u_alloc( sizeof( bl_t ), o->flags, 0, NULL );
        bcore_hnode_tpuz_s* buf_nodes = o->nodes;
        uz_t  buf_size = o->size;
        o->depth_limit = ( o->size > 0 ) ? o->depth_limit + 1 : 4;
        o->size        = ( o->size > 0 ) ? o->size * 2        : 8;
        o->nodes = bcore_u_memzero( sizeof( bcore_hnode_tpuz_s ), NULL, o->size );
        for( uz_t i = 0; i < buf_size; i++ )
        {
            if( buf_nodes[ i ].key ) bcore_hmap_tpuz_s_set( o, buf_nodes[ i ].key, buf_nodes[ i ].val );
        }
...
static void tpuz_s_set_data( bcore_hmap_tpuz_s* o, sr_s data )
{
    ...
    const bcore_hnode_tpuz_s* src = bcore_array_r_get_c_data( &data );
    for( uz_t i = 0; i < size; i++ ) bcore_hmap_tpuz_s_set( o, src[ i ].key, src[ i ].val );
    sr_down( data );
}

//----------------------------------------------------------------------------------------------------------------------

static bcore_self_s* hmap_tpuz_s_create_self( void )
{
    sc_t def = "\
        bcore_hmap_tpuz_s = bcore_inst \
            { \
                aware_t _; \
...
static st_s* hmap_tpuz_selftest( void )
{
...

    {
        ...
        for( uz_t i = 0; i < cycles; i++ )
        {
            ...
            if( ( ( rval1 >> 10 ) & 1 ) == 1 )
            {
                ...
                bcore_hmap_tpuz_s_remove( map, kv.key );
                if( bcore_hmap_tpuz_s_get( map, kv.key ) )  ERR( "deleted key still exists (%lu)", kv.key );
                kvbuf_size--;
                kvbuf[ idx ] = kvbuf[ kvbuf_size ];
            }
        }
    ...
    time = clock();
    bcore_hmap_tpuz_s* map3 = bcore_life_s_push_aware( l, bcore_hmap_tpuz_s_create() );
    ...
    {
        tp_t key = bcore_hmap_tpuz_s_idx_key( map, i );
        if( key != 0 )
        {
            uz_t val = bcore_hmap_tpuz_s_idx_val( map, i );
            ASSERT( bcore_hmap_tpuz_s_exists( map3, key ) );
            ASSERT( val == *bcore_hmap_tpuz_s_get( map3, key ) );
        }
    }
    time = clock() - time;
    ...
    for( uz_t j = 0; j < read_cycles; j++ )
    {
        for( uz_t i = 0; i < kvbuf_size; i++ )
        {
            if( *bcore_hmap_tpuz_s_get( map, kvbuf[ i ].key ) != kvbuf[ i ].val ) ERR( "value mismatch (%lu vs %lu)", *bcore_hmap_tpuz_s_get( map, kvbuf[ i ].key ), kvbuf[ i ].val );
        }
    }
    time = clock() - time;
    st_s_pushf( log, "(%gs per access)\n", ( ( double )time/CLOCKS_PER_SEC ) / ( kvbuf_size * read_cycles ) );
...
static bcore_self_s* hnode_tpsz_s_create_self( void )
{
    ...
    "{"
        ...
        "uz_t val;"
    "}";
    return BCORE_SELF_S_BUILD_PARSE_SC( def, bcore_hnode_tpsz_s );
}
...
sz_t bcore_hmap_tpsz_s_idx_val( const bcore_hmap_tpsz_s* o, uz_t idx )
{
    assert( idx < o->size );
    return o->nodes[ idx ].val;
}
...
static bcore_self_s* hmap_tpsz_s_create_self( void )
{
    ...
    bcore_self_s* self = BCORE_SELF_S_BUILD_PARSE_SC( def, bcore_hmap_tpsz_s );
...
static st_s* hmap_tpsz_selftest( void )
{
...

    {
        ...
        for( uz_t i = 0; i < cycles; i++ )
        {
            ...
            if( kv.val != *val_ptr ) ERR( "value mismatch (%li vs %li)", kv.val, *val_ptr );
    ...
    for( uz_t j = 0; j < read_cycles; j++ )
    {
        for( uz_t i = 0; i < kvbuf_size; i++ )
        {
            if( *bcore_hmap_tpsz_s_get( map, kvbuf[ i ].key ) != kvbuf[ i ].val ) ERR( "value mismatch (%li vs %li)", *bcore_hmap_tpsz_s_get( map, kvbuf[ i ].key ), kvbuf[ i ].val );
        }
    }
    time = clock() - time;
    st_s_pushf( log, "(%gs per access)\n", ( ( double )time/CLOCKS_PER_SEC ) / ( kvbuf_size * read_cycles ) );
...
vd_t bcore_hmap_signal_handler( const bcore_signal_s* o )
{
    switch( bcore_signal_s_handle_type( o, typeof( "bcore_hmap" ) ) )
    {
        case TYPEOF_init1:
        {
            bcore_flect_define_creator( TYPEOF_bcore_hmap_u3vd_s      , hmap_u3vd_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hnode_tpuz_s     , hnode_tpuz_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hmap_tpuz_s      , hmap_tpuz_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hnode_tpsz_s     , hnode_tpsz_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hmap_tpsz_s      , hmap_tpsz_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hnode_tpfp_s     , hnode_tpfp_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hmap_tpfp_s      , hmap_tpfp_s_create_self  );
            bcore_flect_define_creator( TYPEOF_bcore_hnode_tptp_s     , hnode_tptp_s_create_self  );
