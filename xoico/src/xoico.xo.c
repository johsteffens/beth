//  Last update: 2022-11-03T17:03:01Z
...
// HKEYOF_xoico 0x3C65B80BC6B52DA3ull
...
er_t xoico_func_s_parse( xoico_func_s* o, const xoico_host* host, x_source* source )
{
    ...
    bl_t is_enclosed_signature = x_source_parse_bl(source," #?'('" ); // old style enclosed signature
    ...
    else if( xoico_signature_s_parse(signature,host, source ) != 0 )
    {
...
er_t xoico_group_s_finalize( xoico_group_s* o, const xoico_host* host )
{
    ...
    /// default features
    BLM_TRY(xoico_group_s_push_default_feature_from_sc(o,"d @* clone( c @* o );" ))
    BLM_TRY(xoico_group_s_push_default_feature_from_sc(o,"void copy( m @* o, c @* src );" ))
    BLM_TRY(xoico_group_s_push_default_feature_from_sc(o,"void discard( m @* o );" ))
    BLM_TRY(xoico_group_s_push_default_func_from_sc(o,"d obliv @* t_create( tp_t t );" ))
...
er_t xoico_stamp_s_push_default_funcs( xoico_stamp_s* o )
{
    ...
    BLM_TRY(xoico_stamp_s_push_default_func_from_sc(o,"bcore_stamp_funcs.create;" ))
...
er_t xoico_source_s_parse_x( xoico_source_s* o, const xoico_host* host, x_source* source, sc_t group_name, sc_t trait_name )
{
    ...
    if( group )
    {
        ...
        {BLM_INIT_LEVEL(2);
            bcore_arr_st_s_push_st(&(group->explicit_embeddings),((st_s*)BLM_LEVEL_T_PUSH(2,st_s,st_s_create_fa("#<sc_t>.#<sc_t>", o->name.sc, o->ext.sc ))) );
        BLM_DOWN();}
    }
    else
    {
...
er_t xoico_target_s_parse_from_path( xoico_target_s* o, sc_t source_path, sc_t group_name, sc_t trait_name )
{
    ...
    }}if( !source_exists )
    {
        ...
        st_s_copy_sc(&(xsource->ext),bcore_file_extension(source_path ) );
    
        if( bcore_file_exists(source_path ) )
        {
            if( group_name )
            {
                BLM_TRY(xoico_source_s_parse_x(xsource,((const xoico_host*)(o)),((x_source*)( ((bcore_source*)BLM_LEVEL_A_PUSH(0,bcore_file_open_source(source_path ))))), group_name, trait_name ))
            }
            else
            {
                BLM_TRY(xoico_source_s_parse_h(xsource,((const xoico_host*)(o)),((x_source*)( ((bcore_source*)BLM_LEVEL_A_PUSH(0,bcore_file_open_source(source_path ))) ))))
            }
        }
        else
        {
...
er_t xoico_target_s_expand_heading( const xoico_target_s* o, sz_t indent, x_sink* sink )
{
    ...
    x_sink_push_fa(sink," *  Compiling Agent : XOICO (C) 2020 ... 2022 J.B.Steffens\n" );
...
er_t xoico_compiler_s_parse( xoico_compiler_s* o, sc_t target_name, sc_t target_ext, const st_s* target_output_folder, sc_t source_path, sc_t group_name, sc_t trait_name, sz_t* p_target_index )
{
    ...
    BLM_TRY(xoico_target_s_parse_from_path(target,source_path, group_name, trait_name ))
...
er_t xoico_che_s_trans_expression( xoico_che_s* o, x_source* source, xoico_che_result* result_out, xoico_typespec_s* out_typespec )
{
    ...
    if( tp_identifier )
    {
        ...
        if( xoico_che_s_is_builtin_func(o,tp_identifier ) )
        {
            ...
            BLM_TRY(xoico_che_s_trans_builtin(o,tp_identifier, source, NULL, NULL,((xoico_che_result*)( result)), typespec_builtin ))
        ...
        else if( xoico_che_s_is_func(o,tp_identifier ) )
        {
            ...
            if( x_source_parse_bl(source," #=?'('" ) ) // actual function call
            {
                ...
                BLM_TRY(xoico_che_s_trans_function(o,source, func, NULL, NULL,((xoico_che_result*)( result)), typespec_ret ))
...
er_t xoico_che_s_trans_typespec_member( xoico_che_s* o, x_source* source, xoico_che_result* result, const xoico_typespec_s* in_typespec, xoico_typespec_s* out_typespec )
{
    ...
    {BLM_INIT_LEVEL(1);
        ...
        bl_t try_member_function = x_source_parse_bl(source,"#=?'('" );
        ...
        if( try_member_function )
        {
            ...
            {
                xoico_typespec_s* typespec_builtin = ((xoico_typespec_s*)BLM_LEVEL_T_PUSH(0,xoico_typespec_s,xoico_typespec_s_create()));
                BLM_TRY(xoico_che_s_trans_builtin(o,tp_identifier, source, result, in_typespec,((xoico_che_result*)( result_local)), typespec_builtin ))
            ...
            {BLM_INIT_LEVEL(4);
                ...
                xoico_che_result_a_clear(result);
                BLM_TRY(xoico_che_s_trans_function(o,source, info->func, result_object_expr, in_typespec, result, typespec_ret ))
...
{
    // xoico_che_trans_typespec.x:261:1
    BLM_INIT_LEVEL(0);
    BLM_TRY(x_source_parse_fa(source,"[" ))
...
{
    // xoico_che_trans_typespec.x:302:1
    BLM_INIT_LEVEL(0);
    BLM_TRY(x_source_parse_fa(source,"!" ))
...
{
    // xoico_che_trans_typespec.x:341:1
    BLM_INIT_LEVEL(0);
    BLM_TRY(x_source_parse_fa(source,"?" ))
...
{
    // xoico_che_trans_typespec.x:375:1
    BLM_INIT_LEVEL(0);
    if( in_typespec->indirection != 1 )
    {
...
{
    // xoico_che_trans_typespec.x:441:1
    BLM_INIT_LEVEL(0);
    BLM_TRY(x_source_parse_fa(source,"=" ))
    BLM_TRY(xoico_che_result_a_push_sc(result,"=" ))
...
{
    // xoico_che_trans_typespec.x:496:1
    BLM_INIT_LEVEL(0);
    if( out_typespec ) out_typespec->type = 0;
    BLM_TRY(xoico_che_s_trans_whitespace(o,source, result ))
...
er_t xoico_che_s_trans_function( xoico_che_s* o, x_source* source, const xoico_func_s* func, const xoico_che_result* result_object_expr, const xoico_typespec_s* typespec_object, xoico_che_result* result, xoico_typespec_s* return_typespec )
{
    ...
    BLM_TRY(xoico_che_s_trans_function_args(o,source, func, result_object_expr, typespec_object,((xoico_che_result*)( result_args)), typespec_ret ))
...
er_t xoico_builder_target_s_build( xoico_builder_target_s* o )
{
    ...
    {BLM_INIT_LEVEL(4);
        ...
        while( !x_source_eos(source) ) st_s_push_char(&(file_path),x_source_get_char(source) );
        ...
        if( group_name.size == 0 && st_s_ends_in_sc(&(file_path),".x" ) )
        {BLM_INIT_LEVEL(5);
            st_s_copy(&(group_name),((st_s*)BLM_LEVEL_T_PUSH(5,st_s,bcore_file_strip_extension(bcore_file_name(file_path.sc ) ))) );
        ...
        {
            if( !st_s_ends_in_sc(&(file_path),".h" ) )
            {
                BLM_RETURNV(er_t, x_source_parse_error_fa(source,"File name should have extension *.h" ))
            }
            BLM_TRY(xoico_compiler_s_parse(o->compiler,o->name->sc, o->extension->sc, xoico_builder_target_s_root_output_folder(o), file_path.sc, NULL, NULL, (&(index)) ))
        }
        else
        {
            if( !st_s_ends_in_sc(&(file_path),".x" ) )
            {
                BLM_RETURNV(er_t, x_source_parse_error_fa(source,"File name should have extension *.x" ))
            }
            BLM_TRY(xoico_compiler_s_parse(o->compiler,o->name->sc, o->extension->sc, xoico_builder_target_s_root_output_folder(o), file_path.sc, group_name.sc, trait_name.sc, (&(index)) ))
        }
...
// XOICO_BODY_SIGNATURE 0x31F3EC13F9D7CB16
// XOICO_FILE_SIGNATURE 0xAF935A427A76F0B9
